...
test('test', async ({ page }) => {
  ...
  page.on('response', async (response) => {
    ...
    if (url.includes('https://sau.eaglelionsystems.com/v1.0/chatbirrapi/ldap/otpForAttachPhone')) {
  ...
  await page.getByPlaceholder('Search Customer by name,').fill('yonatan million alemu');
  await page.getByRole('row', { name: '339488306847207 Yonatan' }).getByRole('button').click();
  await page.locator('div').filter({ hasText: /^Attach Phone NumberAttach new phone number to the customer superApp account\.$/ }).nth(1).click();

  await page.getByRole('spinbutton', { name: 'Phone Number +' }).click();
  await page.getByRole('spinbutton', { name: 'Phone Number +' }).fill('941301503');
