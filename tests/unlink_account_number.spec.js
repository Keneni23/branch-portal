...
test('test', async ({ page }) => {
    ...
    page.on('response', async (response) => {
      ...
      if (url.includes('https://sau.eaglelionsystems.com/v1.0/chatbirrapi/ldap/sendUnLinkOTP')) {
  ...
  await page.locator('div').filter({ hasText: /^Unlink Account NumberUnlink the customers bank account\.$/ }).nth(1).click();
  await page.locator('div').filter({ hasText: /^2984847921111$/ }).nth(1).click();

  await page.getByRole('button', { name: 'Send UnLink OTP' }).click();
...
/*
  ...
  await page.locator('.eagle-ui-pin-code-field').first().fill('8');
  await page.locator('input:nth-child(2)').fill('9');
  await page.locator('input:nth-child(3)').fill('2');
