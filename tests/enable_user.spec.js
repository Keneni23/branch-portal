...
test('test', async ({ page }) => {
    ...
    page.on('response', async (response) => {
      ...
      if (url.includes('https://sau.eaglelionsystems.com/v1.0/chatbirrapi/ldap/enableInit')) {
  ...
  await page.getByPlaceholder('Search Customer by name,').fill('5019798591021');
  await page.getByRole('row', { name: '8070448012227 019798591' }).getByRole('button').click();
  await page.getByRole('heading', { name: 'Enable User' }).click();
  await page.getByRole('textbox', { name: 'Reason' }).click();
  await page.getByRole('textbox', { name: 'Reason' }).fill('test auto');
...
/*
  ...
  await page.locator('.eagle-ui-pin-code-field').first().fill('4');
  await page.locator('input:nth-child(2)').fill('8');
  await page.locator('input:nth-child(3)').fill('7');
  await page.locator('input:nth-child(4)').fill('0');
  await page.locator('input:nth-child(5)').fill('6');
